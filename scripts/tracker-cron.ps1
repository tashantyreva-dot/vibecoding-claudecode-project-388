...
# Поэтому GITHUB_PAT, TELEGRAM_BOT_TOKEN и TELEGRAM_CHAT_ID читаются НЕ из текста команд,
# а изнутри Node-кода (process.env) — headless-агент просто вызывает
# `node scripts/github-sync.js ...` и `node scripts/run-tracker.js ...` без единого
# символа `$` в командной строке.
#
...
# setx TELEGRAM_CHAT_ID "..."). GitHub PAT — аналогично, в переменной GITHUB_PAT
# (setx GITHUB_PAT "..."). Если какой-то переменной нет — скрипт не запускает прогон
...
НЕ пиши в Bash-командах ничего вида $GITHUB_PAT, $TELEGRAM_BOT_TOKEN, $TELEGRAM_CHAT_ID,
$env:..., ${...} — такие команды блокируются защитой от утечки секретов ещё до выполнения.
Все обращения к GitHub и Telegram уже реализованы в scripts/github-sync.js и
scripts/run-tracker.js — они сами читают токены из process.env изнутри Node-кода. Твоя
задача — вызывать эти скрипты голыми командами (без единого символа `$` в команде) и
передавать между ними пути файлов.

Сделай сегодняшний прогон трекера цен MINI Countryman по .claude/skills/tracker/SKILL.md:
1) `node scripts/github-sync.js get-config <products.yaml>` — скачает products.yaml
   из tashantyreva-dot/vibecoding-claudecode-project-388 во временный файл, который ты
   укажешь. Код выхода 2 = GITHUB_PAT не задан/невалиден — в этом случае сразу останови
   выполнение (см. п.5).
2) `node scripts/github-sync.js get-prev-run <outFile>` — скачает самый свежий прошлый
   ...
   шаге 3 не передавай.
3) `node scripts/run-tracker.js --products <из шага 1> --out <новый файл> [--prev <из шага 2, если не NONE>]` — БЕЗ --dry-run. Дождись завершения (обход сайтов занимает несколько минут). При значимых изменениях скрипт сам отправит Telegram (токен и chat_id читает из process.env сам).
